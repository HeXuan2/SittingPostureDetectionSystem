...
namespace SittingPostureDetectionSystem.DataStatistics
{
    public static class All
    {
        ...
        public static async void Update(Types types, Status status)
        {
            ...
            {
                Types lastTypes = await IO_Operations.IORead<Types>("allTypes");
                ...
                Status lastStatus = await IO_Operations.IORead<Status>("allStatus");
                status.NormalTime += lastStatus.NormalTime;
                status.AbnormalTime += lastStatus.AbnormalTime;
            }

            await IO_Operations.IOWrite("allTypes", types);
            await IO_Operations.IOWrite("allStatus", status);

            await ManageLastTime.UpdateLastTime("allTime");
        }
